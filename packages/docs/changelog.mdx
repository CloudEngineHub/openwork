---
title: "Changelog"
"og:description": "OpenWork release notes: new features, improvements, and bug fixes."
---
<Update label="September 21st" tags={["🐛 Bug Fixes", "🚀 New Features", "🏗️ Refactoring"]}>

  ## [v0.18.49](https://github.com/different-ai/openwork/compare/v0.18.48...v0.18.49): Agent sessions recover more reliably and provider connections stay consistent

  - Conversations now reopen and continue more reliably, with history pages filling correctly and background streaming preserved.
  - Mobile chat is easier to use, with steadier submission, scrolling, keyboard, composer, and sidebar behavior.
  - Provider connections and cloud workspace startup now handle credentials, readiness, and recovery more reliably.
  - Removed custom agents and added clearer linked task references and alpha build information.

</Update>

<Update label="September 9th" tags={["🐛 Bug Fixes", "🏗️ Refactoring"]}>

  ## [v0.18.44](https://github.com/different-ai/openwork/compare/v0.18.43...v0.18.44): Sessions preserve live progress and drafts

  - Sessions now preserve live progress when cached threads are restored.
  - Composer drafts stay in place, and instant sends are verified.
  - Shared callback modes are recorded correctly for isolated MCP rows.
  - Removed the legacy embedded connection app and gateway launch path.

</Update>

<Update label="September 9th" tags={["🚀 New Features", "🐛 Bug Fixes", "🏗️ Refactoring"]}>

  ## [v0.18.43](https://github.com/different-ai/openwork/compare/v0.18.42...v0.18.43): Desktop connections unify and agent work recovers more reliably

  - Desktop connection status now uses one clear native card, while sign-in and managed credentials recover more reliably.
  - Browser tasks can use site tools, page controls, tab suspension and reload, selected logins, and user takeover.
  - Agent runs recover more smoothly from stalled sends, interruptions, restarts, and follow-up turns, with drafts and sent messages staying visible.
  - Computer Use adds hosted approval, native previews, scoped sessions, and continuation after takeover or refresh.
  - Organization access, workflow apps, analytics, onboarding, and desktop policies are clearer and more dependable.

</Update>

<Update label="September 2nd" tags={["🚀 New Features", "🐛 Bug Fixes", "🏗️ Refactoring"]}>

  ## [v0.18.42](https://github.com/different-ai/openwork/compare/v0.18.41...v0.18.42): MCP connections become standard servers and desktop flows stay reliable

  - MCP connections can now be exposed directly as standard MCP servers.
  - OpenWork Dashboard now has its own page and interactive preview, with a new guide for MCP App widgets.
  - Fixed browser opening, narrow-window Settings navigation, session visibility, permission requests, image lightboxes, and queued drafts so everyday work stays reliable.
  - Added clearer Automations examples and Google SSO and SCIM guidance, while SSO setup now validates its configuration before enforcement.
...
<Update label="August 3rd" tags={["🚀 New Features", "🐛 Bug Fixes"]}>
  ...
  - Extensions is now the Library — one place for everything your org can use, with tabs for what's ready, needs sign-in, or needs admin setup, on desktop and web.
  - Any organization member can now create plugins and cloud skills and share them straight from chat; access panels show who can use what, team by team.
  - Organizations can add multiple named Google Workspace connectors — each member signs in once per connector and agents pick the right account by name.
  - Chat now renders LaTeX math, shows immediate feedback while submitting, and adds number shortcuts to jump between visible sessions.
  - Recycled cloud sandboxes keep their chat history, and the artifact panel refreshes when the agent edits the open file.

</Update>

<Update label="July 30th" tags={["🐛 Bug Fixes"]}>

  ## [v0.18.12](https://github.com/different-ai/openwork/compare/v0.18.11...v0.18.12): Managed provider credentials reach the engine

  - Fixed managed cloud provider credentials so they reach the engine's auth API and org-approved models authenticate right after connecting.
  - Fixed the release pipeline to redeploy den-api when the sandbox snapshot pin changes.

</Update>

<Update label="July 29th" tags={["🐛 Bug Fixes", "🚀 New Features"]}>

  ## [v0.18.11](https://github.com/different-ai/openwork/compare/v0.18.10...v0.18.11): Live agent runs stop getting interrupted

  - Credential changes and background provider updates no longer tear down the agent event stream mid-run.
  - Composer skills, MCPs, and extensions load instantly from local data while cloud results land live.
  - Hardened Den organization lifecycles: invitations, re-invites, team cleanup, and legacy SSO/SCIM migration.
  - A malformed workspace list can no longer make the app unloadable.

</Update>

<Update label="July 29th" tags={["🚀 New Features", "🐛 Bug Fixes"]}>

  ## [v0.18.10](https://github.com/different-ai/openwork/compare/v0.18.9...v0.18.10): Org connections come to the desktop and cloud providers get dependable

  - Connect, reconnect, and disconnect organization connections directly from the desktop app.
  - Cloud LLM providers are now set up server-side and reach the engine globally, so org-approved models just work.
  - Cloud workspaces show status and version with a one-click update, and boot states replace stale error flashes.
  - Fixed organization invite lifecycles and redesigned the bring-your-own-key provider editor.

</Update>

<Update label="July 28th" tags={["🐛 Bug Fixes"]}>

  ## [v0.18.9](https://github.com/different-ai/openwork/compare/v0.18.8...v0.18.9): Linux installs repair themselves

  - Linux AppImages now integrate with the desktop, and the launcher repairs itself silently after updates — even on slow networks.
  - Den workers adopt an existing Daytona sandbox instead of getting stuck provisioning forever.
