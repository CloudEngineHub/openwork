  ...
  return {
    ...
    affordances: [
      ...
      affordance({
        ...
        description: "Read recent messages from a session without opening it. The result also carries `status` (idle, busy, retry, waiting), `working` (check it before session.archive), and `model` ({ providerId, modelId, variant } the session is bound to, variant being its reasoning effort; null before a model is bound).",
        provider,
        arguments: [
